...
func listParentAccessApplications(ctx context.Context, d *plugin.QueryData, h *plugin.HydrateData) (interface{}, error) {
	...
	if err := iter.Err(); err != nil {
		...
		if errors.As(err, &cloudFlareErr) {
			if helpers.StringSliceContains([]string{cloudFlareErr.Message}, "Access is not enabled. Visit the Access dashboard at https://dash.cloudflare.com/ and click the 'Enable Access' button.") {
